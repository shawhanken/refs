...
- `/tmp/basefee_sim.py` — 波动负载（实验 1 合并版）
- `/tmp/basefee_sim_split.py` — 拆分版 cycles / cells（实验 1、2）
- `/tmp/basefee_throttle.py` — 节流四维评估（实验 3）

所有脚本的 `update_one` 函数都是 `node/execution/src/basefee.rs` 的整数算术逐行移植。审计可直接对照。
