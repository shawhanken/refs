---
title: "CIP-16: Custom Domains and First-Party TLDs (v2.r2)"
description: Code-aligned v2 — MajorityVote DNS verification, system-mediated callback, explicit migration, fee-charged reverification, corrected status codes, verified-fqdn injection; r2 paired with CIP-14 v2.r2 address shift
---
...
> **Revision history**
>
> - **r2 (2026-05-11)** — Paired with CIP-14 v2.r2 system actor address shift around `SESSION_ACTOR = 0x0C` (already committed in code at `system_actors.rs:35`). `ROUTE_REGISTRY` references throughout Part II shifted `0x0C` → **`0x0D`**; the corresponding GATEWAY_REGISTRY references shifted `0x0D` → **`0x0E`**. Part III §1 system actor table updated. No semantic change to DNS verification / external FQDN protocol.
> - **r1 (2026-04-21)** — Initial v2 alignment round 6 (MajorityVote DNS verification, system-mediated callback, etc.).
>
...
| §4 | `cip-14-dns-addressable-actors-v2.md` (Part II) | Route Registry exists at `0x0D` with the schema in §3 below |
...
`fqdn` is computed from the existing `name` field as `name || ".cowboy.network"`. No records are deleted or moved. Migration runs once at the protocol upgrade boundary (system actor at `0x0D` reads its own state and writes back the augmented schema in a single block).
...
The current low-byte sequence (`node/types/src/constants.rs`, `node/runner/src/system_actors.rs:11-33`) ends at `0x0C` (`SESSION_ACTOR`). The aligned drafts continue the same dense sequence rather than jumping into the `0x10`+ range used by the original CIP-14 (`0x0011`, `0x0012`).
...
| `0x12` | `STREAM_KEY_MANAGER` (CIP-7 r2 §4) | new (r2: shifted from `0x06`) |
