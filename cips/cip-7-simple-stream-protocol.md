...
> **⚠️ [2026-08-16] STALE SNAPSHOT — 过时快照.** This `refs/cips/` copy is superseded; the authoritative, code-aligned spec lives at **`cowboy/docs/cips/cip-7-simple-stream-protocol.md`** (which has since diverged by hundreds of lines). Notably the **Stream Key Manager system-actor address is wrong here**: this copy says `0x12`, but code (`node/runner/src/system_actors.rs`, pin test `well_known_low_byte_assignments`) and the authoritative doc place **`STREAM_KEY_MANAGER` at `0x0D`** — which in turn shifted ROUTE→`0x0E` / GATEWAY→`0x0F` / RECEIPT→`0x10` / PAYMENT_GATE→`0x12` / CONTAINER→`0x13`. See `cowboy/docs/` and [`refs/wiki/entities/system-actors.md`](../wiki/entities/system-actors.md). The `0x12` references below are retained as historical record — do not build against them.
