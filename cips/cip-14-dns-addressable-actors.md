---
title: "CIP-14: DNS-Addressable Actors (v2.r2)"
description: Code-aligned v2 — real PVM syscalls, real RPC primitives, real address space (shifted +1 around SESSION_ACTOR=0x0C), real entitlement registry / settlement plumbing
---
...
> **Revision history**
>
> - **r2 (2026-05-11)** — System actor address segment shifted +1 to make room for `SESSION_ACTOR = 0x0C` (already committed in code at `node/runner/src/system_actors.rs:35`). `ROUTE_REGISTRY` 0x0C → **0x0D**, `GATEWAY_REGISTRY` 0x0D → **0x0E**, `RECEIPT_REGISTRY` 0x0E → **0x0F**. All §4 / §6 / §7 / §8 / Part III §1 references updated. Downstream impact (paired bumps): CIP-10 v2.r2 CONTAINER_REGISTRY 0x0F → 0x10; CIP-15 v2.r2 / CIP-16 v2.r2 cross-references; CIP-18.r2 PaymentGate 0x0013 → 0x11; WP-v2 §13 system actor table updated. No PVM / SDK semantic change.
> - **r1 (2026-04-21)** — Initial v2 alignment round 6 (read_handler RPC, system-mediated dispatch, receipt registry, sender authenticity etc.).
>
...
> - **System actor renumbering** `0x0011` / `0x0012` → `0x0D` / `0x0E` / `0x0F` (continues the existing `0x01..0x0C` sequence; `0x0C` is `SESSION_ACTOR` per `system_actors.rs:35`).
> - **Sender authenticity for `"http.request"`.** Actors check `ctx.sender == GATEWAY_REGISTRY=0x0E` (SDK-default in `@http.handler`). An earlier CIP-14 v2 draft proposed PVM-router selector reservation; that proposal is **withdrawn** because it broke router-actor forwarding patterns — see §6.2 Note.
...
| §1 | Part III of this document | Reserve system addresses `0x0D` (ROUTE_REGISTRY), `0x0E` (GATEWAY_REGISTRY), `0x0F` (RECEIPT_REGISTRY) |
...
- Command-path results are stored in `RECEIPT_REGISTRY=0x0F`, not in actor KV with per-request cleanup timers.
...
Sender allowlist: only `GATEWAY_REGISTRY=0x0E`. Dispatch flow (idiom matches the existing `BASEFEE_SYSTEM_ACTOR=0x06` sender check in `node/execution/src/system_instruction.rs`):
...
4. Synthesise an internal `ActorMessage` to `target.http_request` with `ctx.sender = GATEWAY_REGISTRY=0x0E`.
...
Actors implementing the `http.request` selector MUST verify `ctx.sender == GATEWAY_REGISTRY=0x0E`. Any `ActorMessage` carrying that selector with a different sender MUST be rejected by the actor handler.
...
`GET /_cowboy/requests/{request_id}` is a Gateway-intercepted reserved path (CIP-14 §8.6). Gateway reads via the read-handler RPC against `0x0F`, selector `get_receipt`. Status mapping:
...
`complete_receipt` is a new SDK convenience that emits the `complete_receipt` system call against `RECEIPT_REGISTRY=0x0F`. The registry verifies `ctx.sender` equals the receipt's `target_actor`.

---

## 9. Actor handler convention

An actor receiving an `ActorMessage` with selector `"http.request"` MUST verify `ctx.sender == GATEWAY_REGISTRY=0x0E` and reject otherwise (§6.2). The SDK enforces this by default; custom handlers must include it explicitly.
...
```
ROUTE_REGISTRY_ADDRESS              = 0x0D
GATEWAY_REGISTRY_ADDRESS            = 0x0E
RECEIPT_REGISTRY_ADDRESS            = 0x0F
...
`ROUTE_REGISTRY` / `GATEWAY_REGISTRY` / `RECEIPT_REGISTRY` continue the existing `0x01..0x0C` low-byte sequence (Part III of this document §1; `0x0C` is `SESSION_ACTOR`).
...
- New system actors at unused addresses `0x0D` / `0x0E` / `0x0F`.
...
Rationale: keeping the sequence dense matches `system_actors.rs` convention and avoids the appearance of a reserved block. Original CIP-14 v1 numbers (`0x0011` / `0x0012`) are renumbered to `0x0D` / `0x0E`. The `SESSION_ACTOR` row reflects the existing code commit at `system_actors.rs:35` and is the reason the v2 sequence shifted +1 in r2.
...
CIP-14-aligned defines a `RECEIPT_REGISTRY=0x0F` system actor that owns receipt storage and lifetime. See Part II of this document §8 for the full schema. Two key properties:
