...
After a `VerifiedResult` is committed on-chain, any party may challenge the result within the challenge window by submitting a challenge bond:

**Challenge Window Parameters:**
- Challenge window: 15 minutes (900 blocks at ~1s/block)
- Challenge bond: 100 CBY (burned if challenge fails, returned + reward if challenge succeeds)
- Challenge scope: Any objectively verifiable failure within the 15-minute window

A successful challenge reverts the callback, slashes the offending runner(s), and refunds the challenger's bond plus a reward drawn from the slashed stake. A failed challenge burns the bond to deter spam challenges.

#### **11. Randomness Evolution Path**
...
This CIP is fully backwards compatible with the core protocol. System Actor addresses are unchanged (`0x01`–`0x05`, `0x07`; `0x06` is assigned to DualBasefee and must not be used for off-chain runner operations). The `skip_task` interface is removed; existing task submissions that relied on skip behavior will rely on timeout re-selection instead, which provides equivalent liveness guarantees.
