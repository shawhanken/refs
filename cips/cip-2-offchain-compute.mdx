...
The framework consists of five main on-chain components and the off-chain Runner network:

1. **The Job Dispatcher** (`0x0000...0002`): Receives job submissions, runs VRF selection, and manages job lifecycle.  
2. **The Runner Registry** (`0x0000...0001`): Manages Runner registration, staking, capabilities, health, and reputation.  
3. **The Result Verifier** (`0x0000...0003`): Aggregates runner results and delivers verified outcomes via deferred callback.  
4. **The Entitlement Registry** (`0x0000...0007`): Optional Runner pool access control (see §7).  
5. **Off-chain Runners:** External nodes that execute tasks and submit results.
...
| `required_runner_pool` | Option\<PoolId\> | Optional: only Runners holding this Entitlement pool may be selected (§7) |
...
3. Generate VRF seed: seed = HMAC-SHA256(key=block_hash, "runner-select-v2:" || job_id || submitted_at)
...
4. **TEE filter:** If `tee_required`, runner must declare TEE support  
5. **Price filter:** Estimated cost ≤ `job_spec.max_price`  
6. **Concurrency filter:** `active_jobs < max_concurrent_jobs`  
7. **Entitlement filter** *(optional)*: If `required_runner_pool` is set, runner must hold the specified Entitlement (§7)
...
#### **7. Entitlement Pool Integration (Optional)**

Jobs may specify `required_runner_pool: Some(pool_id)` to restrict selection to runners holding a specific Entitlement from the Entitlement Registry (`0x0000...0007`).

This enables:
- **Open** (default): `required_runner_pool = None` — any qualified runner may be selected
- **Whitelist**: `required_runner_pool = Some(TrustedPoolId)` — only runners granted pool membership
- **Compliance-gated**: Only runners in EU-resident or TEE-certified pools
- **Governance-curated**: DAO grants Entitlement pool membership via on-chain governance

Entitlement is checked during candidate list construction (§4, step 7) and does not affect the VRF algorithm itself.
...
| **Deterministic** | N ≥ 2 | Byte-identical match across all results. Requires `tee_required = true` for meaningful guarantees (LLM inference is inherently non-deterministic without TEE + fixed model hash). |
...
* **No private key for VRF seed:** Using `HMAC(block_hash, job_id)` instead of a dispatcher private key eliminates both the hardcoded-key security hole and the single-node selection bias. `block_hash` is consensus-fixed and globally consistent.
...
This CIP is fully backwards compatible with the core protocol. System Actor addresses are unchanged. The `skip_task` interface is removed; existing task submissions that relied on skip behavior will rely on timeout re-selection instead, which provides equivalent liveness guarantees.

---

### **Security Considerations**

* **VRF Grinding:** A submitter could attempt to choose `submitted_at` to influence the seed. Since `seed = HMAC(block_hash, job_id || submitted_at)` and `block_hash` is consensus-fixed before the submitter can observe it, this attack requires pre-computing the block hash — equivalent to breaking the consensus security assumption.
