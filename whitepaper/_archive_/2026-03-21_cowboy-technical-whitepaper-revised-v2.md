...
> **Note.** This whitepaper presents the current architecture and high-level design of the Cowboy protocol. Detailed parameter tables, normative wire formats, and subsystem-level specifications will be developed in future Cowboy Improvement Proposals (CIPs). For architectural rationale and design decisions, see the [Design Decisions Overview](./cowboy-design-decisions.md).

## Abstract

Cowboy is a general-purpose Layer-1 blockchain that combines a **Python-based actor-model execution environment** with a **proof‑of‑stake consensus** and a **market for verifiable off‑chain computation**. Smart contracts on Cowboy are **actors**: Python programs with private state, a mailbox for messages, and chain‑native timers for autonomous scheduling. For heavy tasks like LLM inference, web requests, MCP tool calls, or **containerized** batch jobs, Cowboy integrates a decentralized network of **Runners** who execute jobs and attest to results under selectable trust models: N-of-M consensus, TEEs, and (in V2) ZK-proofs. Jobs MAY attach **encrypted volumes** (client-side encryption, **Steamtrain** storage cluster) and MAY specify **container** execution against an on-chain image allowlist (**§9**, **0x10**).

> **Note.** Detailed parameter tables, normative wire formats, and subsystem-level specifications will be developed in future Cowboy Improvement Proposals (CIPs).
...
- **Steamtrain / attachable volume:** Distributed encrypted object storage: Runners read and write **volumes** via a client library (FUSE + QUIC); storage nodes see ciphertext only. **CapTokens** bind mount rights to a Runner address; **manifest roots** MAY be anchored on-chain via a dedicated registry.
...
- **Verifiable Off‑Chain Compute.** A decentralized Runner marketplace executes jobs (LLM inference, HTTP fetches, MCP tools, custom compute, **container batch jobs**) off‑chain. Jobs MAY mount **encrypted volumes** backed by **Steamtrain** (client-side encryption, CapToken-bound mounts). Runners stake CBY and are selected via VRF. Results are verified under developer‑selected trust models: N‑of‑M quorum, economic bond, TEE attestation, structured matching, semantic similarity, or (in v2) ZK‑proofs. A commit‑reveal protocol with a 15‑minute challenge window and slashing enforces honesty. Container images MUST appear in the on-chain **Container Registry** (**0x10**) unless a governed TEE exception applies.
...
- **Lane separation:** `LANE_TIMER_CYCLES = 2,000,000` (execution lane, 20% of block) is separate from `TIMER_GC_CYCLES` (cleanup lane) so a TTL-expiry storm cannot starve live execution.
...
The deferred-timer anti-starvation boost cited in earlier drafts ("exponential decay") is not implemented in the current timer layer; the target design replaces it with the per-actor fairness weight in §5.1b below.

#### 5.1b Target Design (EIP-1559 hybrid)

Activated by Tier-3 governance proposal at a chosen block height:
...
- **Per-timer cycle cap during auction phase:** `MAX_CYCLES_PER_FIRE_AUCTION_PHASE = 250,000` (= 12.5% of the lane), preventing any single timer from monopolising. The §5.1a 550k cap remains in force during the FIFO phase.
...
- **Invariants retained from §5.1a:** Tx-then-Timer block ordering, per-fire `fee_payer` pre-charge + refund (the priority tip extends `max_cost` but does not change the mechanics), three-path lifecycle, lane separation, system-instruction opcodes 48/49/50, same-block prohibition.
...
**Attachable storage.** Jobs MAY declare `volume_mounts`: Runners that declare storage capability mount encrypted volumes through **Steamtrain** (client-side encryption; manifest roots anchorable on-chain, deferred). This supports secrets, large artifacts, and multi-step tool chains without putting raw payloads in calldata.
...
| **Runner** | 25% | 1.0× | Runner job results, attestations, and related on-chain follow-ups (e.g. volume manifest anchors) |
| **User** | 50% | 1.0× | User‑initiated transactions |

Unused capacity in higher‑priority lanes cascades to lower‑priority lanes. Transactions are tagged by type at submission; the proposer cannot reassign lanes. Each lane's basefee is `lane_basefee = global_basefee × lane_fee_multiplier`; all four multipliers default to `1.0×` at genesis (no lane subsidy or surcharge). The multipliers are governance‑tunable at the lowest tier; see §17.9 for the canonical parameter table.
...
- **Timers (chain‑native):** The following timer primitives are provided:
  ...
  - Timer delivery is **best‑effort**; execution depends on the timer scheduler (see §5.1 for current rules, including the same-block prohibition).
...
| **Runner** | 25%               | High     | 1.0×           | Runner job results, attestations, volume manifest anchors |
...
- Each lane's basefee is `lane_basefee = global_basefee × lane_fee_multiplier`. All four multipliers default to `1.0×` at genesis (no subsidy, no surcharge) and are governance‑tunable at the lowest tier.
...
¹ The aggregator receives `aggregator_bonus_bps = 150` (= 1.5% of gross) of this 89% on successful settlement. The bonus is **carved out of the runner share**, not from the burn or treasury shares — so this row remains the literal 89% to runner‑side recipients.

² Distribution governance lives in `SlashDistribution { burn_bps, submitter_bps, treasury_bps }`. Genesis defaults `(10000, 0, 0)` preserve this row's 100%‑burn commitment (C7); any non‑zero `submitter_bps` or `treasury_bps` requires a Tier‑3 governance proposal that amends C7 in lockstep with this row.
...
|---------|------|---|----------|
| **0x01** | Runner Registry | ✅ in code | Runner registration, staking, capabilities, health, reputation |
| **0x02** | Job Dispatcher | ✅ in code | Job submission, VRF selection, lifecycle management |
| **0x03** | Result Verifier | ✅ in code | Commit-reveal aggregation, verification, callback dispatch |
| **0x04** | Secrets Manager | ✅ in code | On-chain secret storage surface; per-secret records, release-key registry, proxy registry |
| **0x05** | TEE Verifier | ✅ in code | Remote attestation and trusted measurement verification; TEE-trusted-key state |
| **0x06** | DualBasefee | ✅ in code | Protocol dual-metered basefee state storage; **not** used for off-chain runner operations |
| **0x07** | Entitlement Registry | ✅ in code | Runner Pool access control, general RBAC |
...
| **0x0A** | Storage Manager | ✅ in code | On-chain manifest / volume metadata, per-actor configuration |
| **0x0B** | Relay Registry | ✅ in code | Steamtrain relay node registration, staking, slashing |
| **0x0C** | Session Actor | ✅ in code | MPP Session escrow + voucher settlement |
| **0x0D** | Route Registry | 📋 spec-only | DNS-addressable route mapping. Awaits code activation. |
| **0x0E** | Gateway Registry | 📋 spec-only | HTTP ingress Gateway node registration. Awaits code activation. |
| **0x0F** | Receipt Registry | 📋 spec-only | Command-path HTTP result storage. Awaits code activation. |
| **0x10** | Container Registry | 📋 spec-only | On-chain allowlist of authorized OCI image hashes; resource classes; execution policy. Awaits code activation. |
| **0x11** | Payment Gate | 📋 spec-only | HTTP-native payment policy, budgets, passes, subscriptions. Awaits code activation. |
| **0x12** | Stream Key Manager | 📋 spec-only | VM-level payload encryption key escrow for streams. Awaits code activation. |
| **0x13** | Bank Actor | 📋 spec-only | Agent banking primitive (cards, gas-payment routing, policy enforcement). Activation requires extending the reserved band to ≥ 0x13 or using the `0x1D`-style interception pattern; address may be revised in the activation PR. |
| **0x1D** | Event Subscription Actor | ✅ in code (virtual) | Read RPC surface for the on-chain event-hooks bidding market. Not a deployed actor — `pvm_host::call_actor` intercepts calls to `0x1D` and routes them to `execution::event_sub_system_actor::dispatch_rpc`. Defined at `node/types/src/constants.rs:156`. |
...
> - **In code, virtual (`0x1D`).** Uses an interception pattern in `pvm_host::call_actor` — no deployed code, no entry in `system_actors.rs`. The reserved-band check does not apply because `0x1D` is outside `0x01..=0x0F`.
> - **Spec-only (`0x0D–0x13`).** Seven addresses currently reserved for forthcoming subsystems. None are in code yet. Activation requires per-subsystem work: either extending the reserved band and adding `system_actors.rs` consts (for actors that need full deploy semantics) or adopting the `0x1D`-style interception pattern (for thin RPC-only surfaces). Addresses are best-effort placeholders and may be revised in the activation PR.
>
> **Conflict rule:** If this whitepaper conflicts with code (`node/runner/src/system_actors.rs` + `node/types/src/constants.rs`), **code is authoritative**. Detailed subsystem specifications are out of scope for this whitepaper and will be developed in future CIPs.
...
Slashing for proven dishonesty (fabricated results, wrong model). Operational failures result in reputation penalties only. Committees mitigate single‑runner faults. Storage-volume manifest anchoring (deferred) and container image allowlists reduce supply-chain and cross-runner integrity risk; default container network isolation preserves verifiability for N-of-M modes.
...
`committee M`, `threshold N` = **adaptive**: `M = clip(ceil(2 · log₂(N_active) / max(HHI, 0.01)), 3, 9)`, `N = ceil(2M/3)`; recomputed per 3,600‑block epoch with EMA smoothing of HHI. Static fallback values `M = 5, N = 3` apply until the adaptive committee sizing activates. `challenge_window` = 15 min; `challenge_bond` = 100 CBY; `runner_stake_floor` = 10,000 CBY (CBY‑denominated; Decision Register #4 HOLD on CBY‑denominated, no oracle dependency at v1); `dispute_window_blocks` = 75. `reputation_half_life_blocks` = 1,209,600 (~14 days at 1s blocks); `aggregator_eligibility_percentile` = 50 (p50 reputation); `aggregator_bonus_bps` = 150 (1.5% of gross, paid from runner share); `non_reveal_slash_bps` = 2500 (25% of effective stake on proven non‑reveal); `slash_distribution.{burn_bps, submitter_bps, treasury_bps}` = `(10000, 0, 0)` (HOLD per §8.4 C7; Tier‑3 to amend).
...
- **Off‑chain compute:** Native verifiable Runner market (encrypted volumes (deferred), container jobs) vs. external oracles only.
...
- A system actor on Cowboy, `EventListener` (deferred; address TBD — **0x08** is currently assigned to Treasury), SHALL manage these subscriptions when implemented. This actor relies on the bridge validator set to act as a decentralized oracle, monitoring the Ethereum chain for specified events.
...
> **Note:** Intrinsic costs for other runner-related txs (e.g. **JobSubmit** payloads for HTTP, MCP, **Container**, **VolumeAnchorManifest**) will be normative in forthcoming fee-model and off-chain-compute specifications; extend this table as those specs enumerate types.
...
> **Note**: The authoritative gas cost values for all host operations will be defined in the forthcoming fee-model specification. The values shown here are indicative.

| Operation | Base Cost | Variable Cost | Reference |
|-----------|-----------|---------------|-----------|
| `send_message()` | 80 cycles | — | |
| `storage_read()` | 10 cycles | — | |
| `storage_write()` | 50 cycles | — | |
...
### 17.5. State Rent
Accounts exceeding the grace threshold pay ongoing rent. This section provides the operational summary plus the governance monitoring cadence (Decision Register #4: HOLD on CBY-denominated). Full normative parameters will be developed in the forthcoming storage subsystem specification.
...
Blob storage is **not cell-metered**. Provider payments are direct CBY transfers negotiated off-chain. The forthcoming blob-storage specification will define:
...
(CBFS Relay Node economics — per-byte-per-epoch storage rate, the 10 / 2 / 88 burn / challenge-pool / Relay revenue split, the Relay challenge bond, and the slashing schedule — will be specified in the forthcoming Steamtrain / CBFS subsystem document. Retention contracts and CBFS storage are distinct facilities with scope boundaries to be defined there.)

### 17.7. Off-Chain Compute (Runner Marketplace)
Off-chain job execution (LLM, HTTP, MCP, Custom, **Container**) is **not gas-metered** on the cycle/cell meters. Runners operate in a competitive marketplace:
...
| Selection | Actors specify `max_price` (and optional tip) in **JobSpec**; dispatcher matches eligible runners (VRF sortition) |
...
| Verification | Modes (N-of-M, TEE, etc.) per the forthcoming off-chain-compute specification |

The protocol does **not** mandate unit prices for LLM or other job types—terms are market-driven. Detailed volume fees and container policies will be defined in the forthcoming subsystem specifications and runner registry fields.
...
| Runner | 2,500,000 | 25% | 1.0× | Runner job results, attestations, callbacks (incl. volume manifest anchors) |
