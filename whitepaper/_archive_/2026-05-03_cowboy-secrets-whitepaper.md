...
Charles DePue, Cowboy Foundation

May 2026 — Draft v0.1.0
...
- **Committee:** The ordered set of proxies serving a particular release key. Default 3-of-5; configurable up to 16 with `--committee-override`.
...
### 3.3 Differences vs. Existing Secret Stores
...
### 4.1 Centralised Secret Stores (Vault, AWS Secrets Manager, Doppler, 1Password Secrets Automation)
...
### 4.2 Threshold Cryptography for Decentralised Secrets (Lit Protocol, NuCypher / TACo, drand timelock)
...
### 4.3 Multi-Party Computation Secret Sharing (TSS Vaults, Pyrofex, ZenGo)
...
### 4.4 Hardware-Rooted Approaches (TEE-Sealed Vaults, Intel SGX SealedData)
...
### 5.4 DKG (Once Per Account / Per-Secret Override)
...
### 5.5 Partial Sign and Threshold Combine (Runner Side)
...
### 5.7 Reshare via Proactive Secret Sharing
...
```
g_j(0)  = s_j · L_j               -- L_j is the Lagrange coefficient at zero for the chosen
...
### 6.1 On-Chain Component (System Actor `0x04`)
...
### 6.2 Off-Chain Component (`cbssd` Daemon)
...
### 6.3 Runner-Side Client (`secrets-manager` Crate)
...
## 7 1. Identifiers and Canonical Forms
...
struct SecretVersion {
    version:      u64,
    pending:      bool,                    // true between override SetSecret and
...
struct AccountReleaseKey {
    ...
    prior_committee_epochs: Vec<PriorCommitteeEpoch>, // capped at
...
struct ReleaseReceiptSet {
    quorum_epoch: u64,                    // pinned to first accepted receipt's
                                          // serve_epoch
    threshold:    u8,
...
## 9 3. System Instructions (Opcodes 68–81)
...
## 11 5. The Release Path (Normative)
...
## 12 6. The DKG and Reshare Path (Normative)
...
## 15 9. Liveness, Health, and Slashing Fairness
...
## 20 14. v1 Scope and Phase 2

### 20.1 14.1 Landed in v1

- Cryptographic primitives (`cbss-crypto`): IBE encrypt/decrypt, threshold partial-sign and combine, PSS reshare, AAD binding, HPKE inter-proxy share envelopes, sealed share storage.
- Chain skeleton (`0x04`): all 14 system instructions, storage shapes, pairing-verified receipt acceptance, prior-committee grace window, signed `RotateCommittee`, signed `runner_sig` and `proxy_sig` recovery on `SubmitReleaseReceipt`, fail-closed `tee_required` and slash evidence (until verifiers are implemented).
- Runner-side `${KEY}` substitution: `runner-http`, `runner-mcp`, `runner-llm` integration; `SecretBundle` with `Zeroizing` entries; explicit-env subprocess spawn for stdio MCP servers; PVM `os.environ` deny list for declared keys.
- `cbssd` orchestrator state machine for DKG / reshare ceremony progress.

### 20.2 14.2 Deferred to Phase 2

- **Slash evidence verifiers** for the three fault classes (`InvalidPartialReencrypt`, `DkgSabotage`, `Equivocation`). Today the slash handler fail-closes; auto-slash is non-functional until verifiers land.
- **TEE attestation lookup at receipt submission.** Today `tee_required = true` fail-closes; CIP-23 verifier integration is pending.
- **Bias-free DKG** (`blsful::dkg` migration). The current `cbss-crypto` DKG is rolled-from-scratch Pedersen and biasable; mainnet flag-flip requires the swap.
- **`cbssd` network layer.** The QUIC server, chain watcher, `PartialSign` handler, and receipt submitter are not yet implemented. The orchestrator owns the ceremony state machine but has no transport.
- **Threshold-BLS runner client.** The current `secrets-manager::HttpSecretsClient` is a placeholder that calls a single HTTPS endpoint; the production threshold client (QUIC fanout, Lagrange combine, IBE decrypt) is pending.
- **Liveness deadline transition.** The `ExpireLivenessChallenge` instruction and per-block sweep are pending; today `Pending` challenges do not automatically resolve to `Unanswered`.
- **VRF-based committee selection.** Current selection iterates the proxy index in storage order; full VRF selection (CIP-2 randomness beacon) is pending.
- **Per-instruction gas pricing.** Today `charge_cbss_base` is the only meter; the §8 table is the target.
- **`cowboy secrets` and `cowboy proxy` CLI subcommands.** Owner CLI flows are specified; the implementation of the subcommands is pending.
- **OS-keychain operator key sealing.** Today the operator HPKE key is hex on disk at `0o600`; macOS Keychain / Linux secret-service integration is pending.
- **Cross-CIP doc updates.** CIP-2, CIP-6, CIP-9, CIP-13 reference patches per CIP-24 §2.3 are pending.

### 20.3 14.3 Out of Scope (v1)
...
## 21 15. Relationship to the Technical Whitepaper
...
- Consensus, randomness, and networking follow Technical Whitepaper §6. Committee selection uses the QC-derived randomness beacon for VRF (Phase 2).
