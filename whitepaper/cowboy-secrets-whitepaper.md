...
This section is descriptive and non-binding. Normative requirements are in §§1–15.
...
A proxy holding share `s_i` for committee_epoch `e`, on receiving a runner-signed `PartialSignRequest`, validates the request (§5.3), reads `wrap_epoch` from the on-chain `SecretVersion`, derives `I = hash_to_G1(aad)`, and computes:
