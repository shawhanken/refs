...
1. **最小区块间隔**：`min_block_interval`（默认 **500 ms**）。若距离上次出块时间不足，**直接本次不 propose**（返回 `None`）。这是 **端到端延迟的硬下限之一**（测试/产品可调）。  
...
**关键阈值**：`min_block_interval = 500 ms`。若 1000 笔 Transfer 的「执行循环 + Merkle 化」之和超过 500 ms，出块频率将开始下降。
...
| 问题 | 说明 | 影响 |
|------|------|------|
| `min_block_interval` | 默认 500 ms | **出块间隔下限**，简单 Transfer 无法突破 |
...
- 见 `Application::new` / `with_mempool`：`min_block_interval: Duration::from_millis(500)`。  
- 调整该值会直接改变 **出块尝试频率与用户体验**；须在测试网验证对共识组件的影响（文档化后再改生产默认值）。
