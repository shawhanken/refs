...
**审计日期**：2026-05-15
**审计范围**：本次审计对照的白皮书与 CIP，均为根据 Vending Machine 代币设计修订后的版本——已随 PR `cowboyinc/cowboy#110`（分支 `vm-cip-revisions`，commit `728338399307`）提交、待 Cowboy 团队审核，**尚未合入 `main`**；实现代码取审计当日各仓库最新提交。

- **白皮书**：`cowboyinc/cowboy` 仓库 `docs/whitepaper/cowboy-technical-whitepaper.md`（v2.r2，1530 行；Part I v1 + Part II 9 个 Delta + Part III 对齐审计）
- **CIP 文档**：`cowboyinc/cowboy` 仓库 `docs/cips/` 下的 30 篇 CIP 文档；其中 PR #110 修订 9 篇（CIP-1/2/3/4/5/9/12/13/25）、新增 6 篇（CIP-8/17/26/28/29/31）
- **实现代码**：[`cowboyinc/node`](https://github.com/cowboyinc/node)、[`cowboyinc/runner`](https://github.com/cowboyinc/runner)、[`cowboyinc/cbfs`](https://github.com/cowboyinc/cbfs) 三个仓库的 **`main` 分支**（含 `node/examples/`）——审计当日（2026-05-15）各仓 `main` 分支 HEAD 为 `node@a56f989`、`runner@14c4f24`、`cbfs@65d02fb`

> **注**：本次对比的规范基线包含尚未合入的修订（PR #110），部分标记为 ❌/⚠️ 的项属于「修订后的新规范尚未实现」，属预期内，非代码缺陷。
...
白皮书 `cowboy-technical-whitepaper.md` 是顶层规范文档，分三部分：
...
| 0x0D | (未列) | 未分配 | Route Registry (CIP-14 v2.r2)；CIP-28 亦声明 Bank Actor 于此（冲突，见 §3.1） | ❌ 未实现 |
...
> 下表覆盖 28 个 CIP 编号。说明：CIP-15 含两份文档（Gateway 实现 + 公开资产）合并为一行；CIP-24（Secrets Manager）的实现已并入 §2.1 CIP-2 分析（系统 actor `0x04`），故矩阵未单列；CIP-27 / CIP-30 当前不存在。引言所述「30 篇 CIP 文档」按文件数计（24 篇现有 + PR #110 新增 6 篇）。
...
注：CIP-1/7/8/10/11/13/14/15/16/17/18/19/21/22/23/28/29/31 等在代码注释中较少出现，与其实现度对应。
...
> 白皮书引用：`docs/whitepaper/cowboy-technical-whitepaper.md`（Part I v1 / Part II 9 个 Delta / Part III 对齐审计简报）。
