...
`cip-10-runner-containers.md` states Container Registry at `0x11` in **7 places** (frontmatter `description`, header §1 title, §1 body :1059, tables :1070/:1095/:1107, and the intro callout :14). These are the Container Registry's **own** CIP. **✅ RESOLVED (2026-07-14, owner-authorized): all 8 corrected to `0x13` in cowboy#260** (the intro callout/§1 also had their stale "continues after 0x10" rationale replaced with the real next-free reason). This was the last `Container=0x11` holdout; the address table is now fully consistent with deployed code.
