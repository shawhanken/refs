...
- [ ] **Step 2b — Precondition check (finding F5):** confirm the container billing carried in `RunnerResult.data.billing` is **not** part of any cross-runner byte-match (`VerificationMode::Deterministic`, §3.5.2). It cannot be — per-runner cgroup metering already differs, and the existing code already puts divergent metering there — so container jobs are single-runner / billing is excluded from the matched result. Verify this at impl (grep the container job's `VerificationMode`); if any container path were `Deterministic` over `data`, adding CAE bytes would be the *second* thing to break byte-match, not the first, but the plan assumes (and must assert) billing is match-excluded. Add a one-line test/comment recording the assumption.
...
- [ ] **Step 3 — Implement** `billing_attestor.rs` as a mirror of `result_attestor.rs` (`:68`) / `assemble_nitro_cae` (`nitro_cae.rs:162`), but: `scope_id = billing_id`, `req_hash = empty_req_hash()`, `result_hash = billing_result_hash(...)`, `REPORTDATA` via `reportdata_billing`, freshness `now + FRESHNESS_WINDOW_BLOCKS` (`:23`, ≤ 75). **Nonce: mirror the SHIPPED job-CAE nonce scheme exactly** (see review finding **F1**) — `result_attestor.rs` uses `nonce = keccak256(job_id ‖ result_hash)` with **no** consensus beacon `b`, and `verify_result_cae_light` never consults `b` (freshness/replay come from the `FreshnessAnchor` + `seen_nonces`, not the beacon). Define the billing nonce the same shape, `nonce = keccak256(billing_id ‖ billing_result_hash)` — do NOT introduce the spec's `‖ submission_block_hash ‖ b` mix, which the shipped light path has no plumbing for and would make the producer and verifier disagree. (Read `result_attestor.rs:68` at impl time and copy its exact nonce construction.) Then wire `executor.rs:248`: when a TEE billing signer is configured, set `tee_attestation: Some(d_cbor(cae))`; otherwise `None` (unchanged non-TEE path).
...
- [ ] **Step 1 — Test setup (finding F4):** every positive test MUST seed an **Active `MeasurementBinding`** for the test runner in the registry fixture (the light verify requires one, exactly like the job path). A runner with **no** binding — the ordinary non-TEE container runner — makes `verify_billing_cae_light` return `Err`, which is the *intended* funnel to the deferred/dispute path; add a `no_binding_runner_defers` test asserting that.
- [ ] **Step 2 — Failing tests:** (a) a CAE produced by Task-4's attestor over fixed billing fields **verifies** (binding seeded); (b) tamper each field (e.g. `cpu_used_millicores+1`) → `result_hash` mismatch → **reject**; (c) wrong runner (binding/service key mismatch) → **reject**; (d) stale freshness (`generated_at` too old) → **reject**; (e) replayed nonce (`seen_nonces[job_id]`) → **reject**; (f) a **job-result CAE** (scope_id=job_id) presented as a billing CAE → **reject** (scope_id domain separation, finding F7).
- [ ] **Step 3 — Run, expect fail.** FAIL (`verify_billing_cae_light` absent).
- [ ] **Step 4 — Implement as a STANDALONE function — do NOT refactor `verify_result_cae_light` (finding F2).** `verify_result_cae_light` is a shipped, consensus-critical function on the job hot path; sharing a mutable helper risks a silent regression there for zero benefit. Write `verify_billing_cae_light` as its own function (duplication is acceptable and safer than a shared helper), performing the same ordered checks: freshness (1), replay (2), `verify_service_sig` (`:4066`) over `billing_id ‖ empty_req_hash() ‖ result_hash ‖ attest_digest`, Active `MeasurementBinding` for `runner`, measurement whitelist, DCAP quote-sig vs `bound_quote_key`+REPORTDATA (Nitro rests on the service sig). Signature: `verify_billing_cae_light(cae, job_id, runner, result_hash, registry, snapshot, block_height) -> Result<(), TeeVerifyError>`. Leave `verify_result_cae_light` byte-for-byte untouched.
- [ ] **Step 5 — Run, expect pass.** `cargo test -p cowboy-execution -- cbss::` billing cases green; `verify_result_cae_light` tests still green (they were never touched — confirm the diff shows no edit to that function).
...
- [ ] **Step 3 — Implement:** add `BILLING_TEE_IMMEDIATE_ACTIVATION_HEIGHT: u64 = u64::MAX;` + `#[inline] fn billing_tee_immediate_active(h: u64, act: u64) -> bool { h >= act }`. Add `cae_final: bool` to `PendingContainerSettlement`. In `settle_container_compute`, AFTER the existing `metered` decision (`:1099-1113`), if `billing_tee_immediate_active(block_height, ACT)` AND `tee_attestation` present AND `verify_billing_cae_light(...) == Ok`, set `settle_at_block = block_height`, `cae_final = true` (the verified metered cost is authoritative). Otherwise unchanged. The verified path still `.min(lock.amount)` (finding: a verified CAE authenticates the *meter*, never the right to exceed the pre-locked escrow).
- [ ] **Step 3b — State-boundary check (finding F3):** `verify_billing_cae_light` mutates chain state — it inserts the billing nonce into `seen_nonces[job_id]` (replay protection). `settle_container_compute` runs inside the speculative-execution batch (`transaction_executor_impl.rs:109` → per-block; propose/verify roll back). Confirm the nonce insertion rides the SAME batch/write-set that a `rollback` discards, so a re-proposed/rolled-back block does not leave a poisoned nonce that rejects the honest re-submission. `verify_result_cae_light` already lives inside this boundary; keep the billing call at the same layer (do NOT hoist verification outside the settlement transaction). Add a test: verify → rollback → re-verify the same CAE succeeds (nonce not permanently burned).
...
## 7. Deep-review findings (adversarial pass, 2026-07-28)

Ran spec-correctness / anti-spoof / consensus-determinism / blast-radius lenses against this plan + CIP-23 §3.5/§3.8 + the shipped code. Findings, folded into the tasks above:

- **F1 (HIGH, spec-vs-shipped divergence) — billing nonce.** The plan first specified the CIP-23 §3.5 nonce `keccak(billing_fields ‖ submission_block_hash ‖ b)`, but the **shipped** job attestor (`result_attestor.rs:68`) drops the consensus beacon `b` entirely (`nonce = keccak(job_id ‖ result_hash)`) and `verify_result_cae_light` never consults `b`. Mirroring the spec rather than the shipped scheme would make the runner producer and the node verifier compute different nonces → every honest billing CAE rejected, or a demand for beacon plumbing that does not exist on the light path. **Fixed:** Task 4 now mirrors the shipped nonce shape (`keccak(billing_id ‖ billing_result_hash)`), read from `result_attestor.rs` at impl time. *(The spec/shipped nonce divergence is a pre-existing CIP-23 drift worth a separate ticket, but COW-2504 must match what ships, not what the spec says.)*
- **F2 (HIGH, blast radius) — do not refactor `verify_result_cae_light`.** The plan proposed a shared helper between the job and billing light verifiers. Editing a shipped consensus-critical verify to share code with a new path risks a silent job-path regression for no real gain. **Fixed:** Task 5 now writes `verify_billing_cae_light` standalone and leaves the job verify byte-for-byte untouched.
- **F3 (MED, consensus state boundary) — nonce insertion must roll back with settlement.** `verify_billing_cae_light` writes `seen_nonces[job_id]`; if it ran outside the speculative-execution batch that `settle_container_compute` lives in, a rolled-back/re-proposed block would leave a burned nonce that rejects the honest re-submit. **Fixed:** Task 6 Step 3b pins the call inside the settlement transaction + adds a verify→rollback→re-verify test.
- **F4 (MED, testability + intended funnel) — MeasurementBinding fixture.** Billing verify requires an Active binding, exactly like the job path. Positive tests must seed one; a non-TEE runner (no binding) correctly fails verify → deferred/dispute path. **Fixed:** Task 5 Step 1 + a `no_binding_runner_defers` test.
- **F5 (MED, safety assumption) — billing must be match-excluded.** Adding CAE bytes to `data.billing` is safe only because container billing is not part of a cross-runner `Deterministic` byte-match (per-runner metering already differs). **Fixed:** Task 3 Step 2b asserts/records the assumption.
- **F7 (LOW, cross-CAE confusion) — scope_id domain separation.** `billing_id` (scope_id for billing) must differ from `job_id` (scope_id for the job CAE) so a job-result CAE can't be replayed as a billing CAE. `billing_id = keccak(job_id ‖ runner) ≠ job_id` gives this. **Fixed:** Task 5 test (f) asserts a job CAE is rejected as a billing CAE.
- **Positive finding — no float/WP-§207 entanglement.** Billing fields are all integers, so `billing_fields_preimage` has none of the `binary64` canonicalization / governance-blocker exposure that the Job-path §3.5.1a preimage carries. Billing is strictly simpler than the job path here.
